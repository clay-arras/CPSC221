...
string displayRGBAPixel(RGBAPixel &px) {
  string s = "";
  s += (char)px.r;
  s += (char)px.g;
  s += (char)px.b;
...
void displayPNG(PNG &p) {
  std::cout << "Display" << std::endl;
  for (unsigned int i=0; i<p.width_; i++) {
    for (unsigned int j=0; j<p.height_; j++) {
      RGBAPixel* px = p.getPixel(i, j);
      std::cout << "(" << std::setw(3) << (int)px->r << " " << std::setw(3) << (int)px->g << " " << std::setw(3) << (int)px->b << " " << std::setw(3) << (int)px->a << ") ";
    }
    std::cout << std::endl;
  }
}
