...
void remove(Node *&head, int oldKey)
{
    ...
    Node* ptr=head;
    for (; ptr!=NULL && ptr->next!=NULL 
        && ptr->next->key!=oldKey; ptr=ptr->next);
    if (head != NULL && head->next != NULL) {
        Node* tmp = NULL;
        if (ptr->next != NULL)
            tmp = ptr->next->next;
...
{
    Node* ptr=head;
    for (; ptr!=NULL && ptr->next!=NULL && ptr->key!=oldKey; ptr=ptr->next);
    if (ptr->key == oldKey) 
...
Node *interleave(Node *list1, Node *list2)
{
    ...
    Node *head = new Node(list1->key, interleave(list2, list1->next));
    return head;
}
